...
class AuthRemoteDataSource(
    ...
    suspend fun sendEmail(dto: EmailRequest): HttpResponse =
        client
            .post("auth/email/send") {
                setBody(dto)
            }.body()

    suspend fun login(dto: LoginRequest): LoginResponse =
        client
            .post("auth/login") {
                setBody(dto)
            }.body()

    suspend fun sendEmailWithCode(dto: EmailVerifyRequest): EmailVerifyResponse =
        client
            .post("auth/email/verify") {
                setBody(dto)
            }.body()

    suspend fun signUp(dto: SignUpRequest): LoginResponse =
        client
            .post("auth/signup") {
                setBody(dto)
            }.body()

    suspend fun refreshToken(refreshToken: RefreshTokenRequest): RefreshTokenResponse =
        client
            .post("auth/refresh") {
